...
function checkInput2(){
	...
	if (email_flag["order_billing"] ==1 && document.getElementById("order_billing_first_name").value!="" && document.getElementById("order_billing_last_name").value!="" &&  confirm_password==1 ){
		document.getElementById("edit_account_submit").disabled = false;
		$("#edit_account_submit").animate({"opacity":"1"},100);
	}
	else{
		document.getElementById("edit_account_submit").disabled = true;
		$("#edit_account_submit").animate({"opacity":"0.4"},100);
	}
